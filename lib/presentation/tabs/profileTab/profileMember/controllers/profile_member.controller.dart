...
class ProfileMemberController extends GetxController {
  ...
  Future<void> initProfileMember() async {
    loading.value = true;
  ...
  Future<List<Map<String, dynamic>>> getUserList(
      String direction, String field, String status) async {
    printLog('direction->$direction, field->$field, status->$status');
    try {
      final response = await ApiService().get(
          'check-in-record/history?orderByDirection=${direction.toUpperCase()}&checkInRecordOrderByField=${field.toUpperCase()}&filterStatus=${status.toUpperCase()}');
